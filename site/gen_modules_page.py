...
BASECAMP_INSTALL_URL = "https://docs.logos.co/basecamp/install-logos-basecamp"  # install guide
...
# Small inline GitHub mark (currentColor) for "Source" links.
GH_ICON = ('<svg viewBox="0 0 16 16" width="13" height="13" aria-hidden="true" '
           ...
           '1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31'
           '-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27'
...
def _add_link(links, seen, label, url):
    """Append (label, url) once, if url is a real http(s) link not already added."""
    if _is_url(url) and url not in seen:
        seen.add(url)
        links.append((label, url))


def collect_links(ov, source_url="", website_url="", docs_url=""):
    """Ordered, de-duped explicit links for a card. Override keys win/augment:
    `repo`/`source` → Source, `docs` → Docs, `website`/`homepage` → Website. Auto
    values (from a manifest/F-Droid metadata) fill in when the override is silent."""
    links, seen = [], set()
    _add_link(links, seen, "Source", ov.get("repo") or ov.get("source") or source_url)
    _add_link(links, seen, "Docs", ov.get("docs") or docs_url)
    _add_link(links, seen, "Website", ov.get("website") or ov.get("homepage") or website_url)
    return links
...
def build_app_cards(fdroid_index, base_url, repo_dir, overrides, offline):
    ...
    for pkgname, p in (fdroid_index.get("packages") or {}).items():
        ...
        cards.append({
            ...
            # F-Droid metadata already exposes sourceCode / webSite — use them.
            "links": collect_links(
                ov,
                source_url=loc(meta.get("sourceCode")) or "",
                website_url=loc(meta.get("webSite")) or "",
            ),
...
def render_card(c):
    ...
    # Explicit, labeled links (Source / Docs / Website) — so reaching the source
    # doesn't rely on knowing the title is a hidden link.
    links_html = ""
    if c.get("links"):
        parts = "".join(
            f'<a class="lnk" href="{e(u)}" target="_blank" rel="noopener">'
            f'{GH_ICON if label == "Source" else ""}{e(label)}</a>'
            for label, u in c["links"]
        )
        links_html = f'<div class="links">{parts}</div>'
    title = e(c["title"])
    # Link the title to the first explicit link (repo/website) if not overridden.
